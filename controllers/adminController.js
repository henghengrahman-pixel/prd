...
function orderUpdateStatus(req, res) { updateOrderStatus(req.params.id, req.body.status); setFlash(req, 'success', 'Status order diupdate.'); res.redirect('/admin/orders'); }

function articleList(req, res) { res.render('admin/articles', { articles: getArticles() }); }
function articleCreatePage(req, res) { res.render('admin/article-form', { item: null }); }
function articleStore(req, res) { createArticle(req.body); setFlash(req, 'success', 'Artikel berhasil dibuat.'); res.redirect('/admin/articles'); }
function articleEditPage(req, res) { res.render('admin/article-form', { item: getArticleById(req.params.id) || null }); }
function articleUpdate(req, res) { updateArticle(req.params.id, req.body); setFlash(req, 'success', 'Artikel berhasil diupdate.'); res.redirect('/admin/articles'); }
function articleDelete(req, res) { deleteArticle(req.params.id); setFlash(req, 'success', 'Artikel berhasil dihapus.'); res.redirect('/admin/articles'); }

function settingsPage(req, res) { res.render('admin/settings', { settings: getSettings() }); }
function settingsUpdate(req, res) {
  ...
  saveSettings({ ...current, ...payload });
  setFlash(req, 'success', 'Settings berhasil disimpan.');
  res.redirect('/admin/settings');
}

module.exports = {
  loginPage, login, logout, dashboard,
  productList, productCreatePage, productStore, productEditPage, productUpdate, productDelete,
  orderList, orderUpdateStatus,
  articleList, articleCreatePage, articleStore, articleEditPage, articleUpdate, articleDelete,
